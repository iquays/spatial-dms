...
//------------------------------------------------------------------------------------------------//
    ...
    {
        $defaultPetas = Peta::find()->where(['default_on_frontpage' => 1])->all();
        $searchModel = new PetaSearch();
        $petaProvider = $searchModel->search(Yii::$app->request->queryParams);

        $this->layout = 'map-layout';

        return $this->render('peta', [
            'defaultPetas' => $defaultPetas,
            'petaProvider' => $petaProvider,
